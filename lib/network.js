...
Network.prototype.train = function(examples) { // Training with Back propagation algorithm
  ...
  for (var itr = 0; itr < iterations; itr++) { // Repeat the process #iterations
    for (var ex = 0; ex < examples.length; ex++) {
      //console.log("Exemple n°",i);
      var targets = examples[ex][1];
      var input = examples[ex][0];
      ...
      for (var l = this.layers.length - 2 ; l >= 0 ; l--) {
        ...
        for (var j = 0; j < this.layers[l].neurons.length; j++) { // For each neuron in the layer

          var neur = this.layers[l].neurons[j];

          // For each neuron, we look at the neurons of the next layer
          neur.error = math.sum(
            this.layers[l+1].neurons.map(function(n){ return n.weights[j]*n.delta })); // (Delta of neuron #1 in next layer * weight of the branch to that neuron + delta of neuron #2 ...)
